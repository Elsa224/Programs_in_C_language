...
void ajoutEtudiant(FILE *pointeurFichier, etudiant tableau[], int taille)
{
    ...
    printf("\n\nLa saisie obtenue est la suivante :\n\n");
    pointeurFichier = fopen("fichier_BASE.txt", "r");
    lectureFichier(pointeurFichier, tableau, nbStudentInFile("fichier_BASE.txt"));

    //Tri par ordre alphabétique
    triBulleAlphabetique(nbStudentInFile("fichier_BASE.txt"), tableau);

    //Création du fichier FICH_BASE avec ajout
    pointeurFichier = fopen("fichier_BASE.txt", "w");
    creationFichier(pointeurFichier, tableau, nbStudentInFile("fichier_BASE.txt"));
